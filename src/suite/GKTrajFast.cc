...
//
int GKTrajFast::Eccentric(const Real dtstart, TrajData traj[],
...
// Note that dtstart is in radiation reaction time units!
//
int GKTrajFast::Circular(const Real dtstart, TrajData traj[],
		     int StepsMax, int & StepsTaken)
{
  dt = dtstart;
  //
...
//
Real GKTrajFast::P_lso(const Real csi, const Real e)
{
  //
  // Bracket.
  //
...
void GKTrajFast::TakeAStep(Real x[], const Real t1)
{
...
//
int GKTrajFast::odeint(Real ystart[], const int nvar, const Real x1,
...
//
int GKTrajFast::bsstep(Real y[], Real dydx[], int nv, Real *xx,
...
//
void GKTrajFast::Param_derivs(Real t, Real *x, Real *dxdt)
{
  const Real ppp = x[1];
  const Real eee = x[2];
...
//
int GKTrajFast::mmid(Real y[], Real dydx[], int nvar, Real xs,
...
void GKTrajFast::Rroots(const Real Ehere, const Real Lzhere, const Real Qhere,
